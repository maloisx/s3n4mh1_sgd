...
function validarnumerosyletras_sinenter(e) { 
    tecla = (document.all) ? e.keyCode : e.which;
    console.log(tecla);
    //alert(tecla);
    if (tecla==8) return true; //Tecla de retroceso (para poder borrar) 
    if (tecla==0) return false; //Tecla de tab
    if (tecla=='13') return false; //Tecla de enter
    if (tecla==32) return true;
    // dejar la línea de patron que se necesite y borrar el resto 
    //patron =/[A-Za-z]/; // Solo acepta letras 
    //patron = /\d/; // Solo acepta números
...
function ChangeColorrow(tableRow, highLight)
{
    if (highLight)
    {
      tableRow.style.backgroundColor = '#F38994';
    }
    else
    {
...
function diferencias_dias(f1,f2){
	var d1 = f1.split("-");
